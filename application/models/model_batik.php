...
class model_batik extends CI_Model{
    ...
    function tampildata()
    {
        ...
        $query= $this->db->query("SELECT  top 200 * FROM requesttable where (airline='ID' OR airline='JT' or airline='IW') order by logid desc");
    ...
    function tampilperbaris($baris)
    {
        ...
        $query= $this->db->query("SELECT top $baris  * FROM requesttable where (airline='ID' OR airline='JT' or airline='IW') order by logid desc");
        foreach($query->result_array() as $row=> $value){
    ...
    function get_responselion($pnr){
        $sql="SELECT * FROM responselion where pnr='$pnr'";
        $query=$this->db->query($sql);
        foreach($query->result_array() as $row=> $value){
            $data[]=$value;
        }
